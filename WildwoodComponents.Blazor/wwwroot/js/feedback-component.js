...
// Reads selected files into base64 attachments, enforcing size/type limits.
// `inputElement` is the hidden <input type="file"> element reference.
// Returns { accepted: [{name,contentType,size,data}], errors: [string] }.
export async function readAttachments(inputElement, maxAttachmentSizeKb, allowedTypesCsv) {
    const result = { accepted: [], errors: [] };
    if (!inputElement || !inputElement.files) return result;

    const files = inputElement.files;
    const maxSize = (maxAttachmentSizeKb || 2048) * 1024;
    const allowedTypes = (allowedTypesCsv || '').split(',').map(function (t) { return t.trim().toLowerCase(); });
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (file.size > maxSize) {
            result.errors.push(file.name + ' exceeds the ' + (maxAttachmentSizeKb || 2048) + 'KB limit.');
            continue;
        }
        ...
        if (data) {
            result.accepted.push({ name: file.name, contentType: file.type, size: file.size, data: data });
        }
    }

    // Clear the input so the same file can be selected again.
    try { inputElement.value = ''; } catch (e) { /* ignore */ }
    return result;
}
