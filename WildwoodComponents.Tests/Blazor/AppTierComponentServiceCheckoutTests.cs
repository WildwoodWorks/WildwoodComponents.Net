...
/// <summary>
/// Port of packages/wildwood-core/src/__tests__/appTierService.checkout.test.ts, case for case:
/// the request path, verb and exact JSON body keys of every new method, the three error-mapper
/// branches, and the semantics that are easy to get wrong (RequiresAction/Processing are data,
/// SupportsPaymentAction is sent only by the options form, ?immediate= is lowercase).
/// </summary>
public class AppTierComponentServiceCheckoutTests
{
    ...
    private static AppTierComponentService CreateOfflineService()
        => new(new HttpClient(new ThrowingHandler()) { BaseAddress = new Uri("https://api.test/") },
            NullLogger<AppTierComponentService>.Instance);
    ...
    public async Task QuoteAddOnCheckoutAsync_PostsAPascalCaseItemList_AndMapsTheAnswer()
    {
        ...
        Assert.Equal("vault", items[1].GetProperty("AddOnId").GetString());
    ...
    [Fact]
    public async Task CheckoutAddOnsAsync_DefaultsUseSavedCardToFalse_AndKeepsTheCheckoutIdOnARefusal()
    {
        var (service, handler) = CreateService();
    ...
    public async Task CompleteAddOnCheckoutAsync_CompletesOnePack_AndKeepsThePackARefusalWasAbout()
    {
        ...
        Assert.Equal("ap-1", failed.PricingId);
    ...
    public async Task SubscribeToAddOnDetailedAsync_ReturnsTheSubscription_OrAStructuredRefusal()
    {
        ...
        var (refusingService, refusingHandler) = CreateService();
    ...
    public async Task TheDeprecatedBooleanWrappers_StillAnswerTrueOrFalse()
    {
        ...
        var (cancelService, cancelHandler) = CreateService();
        ...
        var (missingService, missingHandler) = CreateService();
    ...
    public async Task ChangeTierAsync_OptionsOverload_PostsSupportsPaymentAction_AndReturnsRequiresActionAsData()
    {
        ...
        Assert.Equal(20m, result.AmountDue);
    }
    ...
    public async Task CompleteTierChangeAsync_PostsTheParkedChangeId_AndReportsRefusalsStructurally()
    {
        ...
        var (expiringService, expiringHandler) = CreateService();
