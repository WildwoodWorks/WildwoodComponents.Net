...
public interface IWildwoodAppTierService
{
    ...
    /// <see cref="SelfChangeTierOptions.SupportsPaymentAction"/>: the positional form posts exactly
    /// what it always did, because an older server rejects an unknown property. A change whose
    /// proration needs 3-D Secure comes back with RequiresAction, a client secret and a
    /// PendingChangeId for <see cref="CompleteTierChangeAsync"/> — Success=false there means
    ...
    /// a failure answers "eligible" with an empty (= unknown) pack map, because the quote and the
    /// payment initiation re-decide authoritatively before any money moves.
    /// </summary>
    ...
    /// period already paid for; <paramref name="immediate"/> ends it now. Never throws.
    /// </summary>
    ...
    /// <summary>
    /// Buy the basket. One pack failing does not stop the others, so read Results per pack rather
    /// than Success alone.
    /// </summary>
    ...
    /// <summary>
    /// The app's public tiers, swallowing failures into an empty list. Kept for existing callers;
    /// a view that must distinguish "sells no plans" from "pricing unavailable" uses
